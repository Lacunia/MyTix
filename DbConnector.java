...
public class DbConnector {
    ...
    public static void main(String[] args) throws SQLException {
        try (Connection conn = DriverManager.getConnection(URL, USER, PASS); Scanner in = new Scanner(System.in)) {
            ...
            while (true) {
                System.out.println("\n1) Browse as guest  2) Login as customer  3) Login as organizer  0) Exit");
    ...
    private static void customerMenu(Connection c, Scanner in, int customerId) throws SQLException {
        ...
        while (true) {
            ...
            switch (in.nextLine().trim()) {
                case "1" -> { int p=performance(c,in); System.out.print("Section: "); String s=in.nextLine(); System.out.print("Row: "); String row=in.nextLine(); System.out.print("Seat numbers: "); booking.bookReservedSeats(customerId,p,payment(c,customerId),seats(c,p,s,row,in.nextLine())); }
                case "2" -> { int p=performance(c,in); System.out.print("GA section name: "); int s=section(c,p,in.nextLine()); System.out.print("Quantity: "); booking.bookGeneralAdmission(customerId,p,payment(c,customerId),s,Integer.parseInt(in.nextLine())); }
    ...
    private static void organizerMenu(Connection c, Scanner in, int organizerId) throws SQLException {
        ...
        while (true) {
            ...
            switch (in.nextLine().trim()) {
                ...
                case "2" -> { int e=ownedEvent(c,in,organizerId); System.out.print("Venue exact address: "); int v=venue(c,in.nextLine()); System.out.print("Performance name: "); String n=in.nextLine(); System.out.print("Date/time (YYYY-MM-DDTHH:MM): "); events.addPerformance(e,v,n,LocalDateTime.parse(in.nextLine())); }
                ...
                case "4" -> { int p=ownedPerformance(c,in,organizerId); System.out.print("Section name: "); int s=section(c,p,in.nextLine()); System.out.print("Tier name: "); int t=tier(c,p,in.nextLine()); events.assignSectionsToTiers(p,Map.of(s,t)); }
                ...
                case "6" -> { int p=ownedPerformance(c,in,organizerId); System.out.print("Tier name: "); int t=tier(c,p,in.nextLine()); System.out.print("New price: "); System.out.println(events.updateTierPrice(p,t,Double.parseDouble(in.nextLine())) ? "Price updated." : "Price cannot change after a sale."); }
                case "7" -> { int p=ownedPerformance(c,in,organizerId); System.out.print("Section: "); String s=in.nextLine(); System.out.print("Row: "); String r=in.nextLine(); System.out.print("Seat number: "); System.out.println(events.blockSeat(p,seats(c,p,s,r,in.nextLine()).get(0)) ? "Seat blocked." : "Seat has been sold."); }
    ...
    private static int performance(Connection c,Scanner in)throws SQLException{System.out.print("Performance name: ");String n=in.nextLine();System.out.print("Date/time YYYY-MM-DDTHH:MM: ");LocalDateTime d=LocalDateTime.parse(in.nextLine());return one(c,"SELECT performanceId FROM Performances WHERE name=? AND dateTime=?",n,Timestamp.valueOf(d));}
    ...
    private static int ownedEvent(Connection c,Scanner in,int o)throws SQLException{System.out.print("Exact event title: ");return one(c,"SELECT eventId FROM Events WHERE title=? AND organizerId=?",in.nextLine(),o);}
    private static int venue(Connection c,String a)throws SQLException{return one(c,"SELECT venueId FROM Venues WHERE address=?",a);}
    ...
    private static List<Integer> seats(Connection c,int p,String section,String row,String ns)throws SQLException{int sec=section(c,p,section);List<Integer>out=new ArrayList<>();for(String n:ns.split(","))out.add(one(c,"SELECT st.seatId FROM Seats st JOIN SectionRows r ON r.rowId=st.rowId WHERE r.sectionId=? AND r.rowName=? AND st.seatNumber=?",sec,row,Integer.parseInt(n.trim())));return out;}
